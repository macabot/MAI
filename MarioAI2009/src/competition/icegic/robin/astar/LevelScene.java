...
import competition.icegic.robin.astar.level.SpriteTemplate;
import competition.icegic.robin.astar.sprites.BulletBill;
import competition.icegic.robin.astar.sprites.Enemy;
import competition.icegic.robin.astar.sprites.FireFlower;
import competition.icegic.robin.astar.sprites.Fireball;
import competition.icegic.robin.astar.sprites.FlowerEnemy;
import competition.icegic.robin.astar.sprites.Mario;
import competition.icegic.robin.astar.sprites.Mushroom;
import competition.icegic.robin.astar.sprites.Shell;
import competition.icegic.robin.astar.sprites.Sprite;
import competition.icegic.robin.astar.sprites.SpriteContext;
...
public class LevelScene implements SpriteContext, Cloneable
{
    ...
    public boolean setLevelScene(byte[][] data)
    {
        ...
        // calculate last bump
        int endHeight = heights[21];
        int lastXheightChanged = 0;
        for(int i = 20; i > 0; i--)
        {
        	if (heights[i] != endHeight)
        	{
        		lastXheightChanged = i;
        		break;
        	}
        }
        lastEventX = Math.max(lastXheightChanged, lastEventX);
        if (lastEventX < 21-7 && MarioXInMap+11 > 264)
        {
        	//System.out.println("Possible level end found! " + (MarioXInMap+lastEventX-2) + " lastEventX: "+lastEventX);
        	levelEndX = (MarioXInMap+lastEventX-2) * 16;
        }
        ...
        if (gapAtLast && !gapAtSecondLast)
        {
        	...
        	{
        		level.setBlock(MarioXInMap + HalfObsWidth + holeWidth, gapBorderHeight, (byte) 4);
        	}
        	return true;
        }
    	return false;
